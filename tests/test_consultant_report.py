...
class TestConsultantReportStructure(unittest.TestCase):
    ...
    def test_header_metadata_present(self):
        ...
        self.assertIn("Generated by Vibe Explainer", self.md)
        self.assertIn("vibe-explainer 0.2.0a1 (schema 2.0)", self.md)
        self.assertIn("penetration test", self.md)
        self.assertIn("does not prove exploitability", self.md)
    ...
    def test_evidence_ids_appear_for_traceability(self):
        ...
        self.assertIn("Concern ID:", self.md)
...
class TestConsultantReportContent(unittest.TestCase):
    def test_default_report_is_unscored_and_enforcement_unknown(self):
        md = _md("agent-with-tools", assessment_date="2026-01-01")
        self.assertIn("unscored concern scenario", md)
    ...
    def test_no_ai_repo_terminates_early(self):
        ...
        self.assertIn("No supported AI-related signal was detected", md)
        # should not render risk/readiness detail sections for a no-AI repo
        self.assertNotIn("## Concern Scenarios", md)
        self.assertNotIn("## Process-Evidence Checklist", md)

    def test_process_checklist_shows_all_four_checks(self):
        md = _md("readiness-managed", assessment_date="2026-01-01")
        for name in (
            "AI inventory and threat-model scope",
            "Repeatable AI security testing",
            "CI-integrated security evaluation",
            "Scheduled evaluation and retained evidence",
        ):
...
class TestConsultantCLI(unittest.TestCase):
    ...
    def test_consultant_flag_produces_markdown(self):
        result = self._run(str(FIXTURES / "agent-with-tools"), "--report")
    ...
    def test_consultant_no_secret_leak(self):
        result = self._run(str(FIXTURES / "hardcoded-credential"), "--report")
        self.assertNotIn("sk-proj-", result.stdout)

    def test_consultant_alias_remains_compatible(self):
        result = self._run(str(FIXTURES / "agent-with-tools"), "--consultant")
        self.assertEqual(result.returncode, 0)
        self.assertIn("# AI Repository Evidence Review", result.stdout)
    ...
    def test_default_and_plain_security_modes_unaffected(self):
        ...
        self.assertIn("AI REPOSITORY EVIDENCE REVIEW", plain.stdout)
